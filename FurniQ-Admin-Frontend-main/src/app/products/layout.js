...
// The ProductSidebar component is defined directly inside the layout file.
function ProductSidebar() {
    const [activeProducts, setActiveProducts] = useState([]);
    const [loading, setLoading] = useState(true);
    const [currentPath, setCurrentPath] = useState('');

    // This ensures API calls work correctly in any environment.
            ...
            <nav>
                <ul className="space-y-2">
                    {activeProducts.map(product => {
                        ...
                        return (
                            <li key={product.id}>
                                <Link href={href} className={`flex flex-col items-center text-center p-3 rounded-md transition-colors ${isActive
...
// This should be the ONLY default export.
