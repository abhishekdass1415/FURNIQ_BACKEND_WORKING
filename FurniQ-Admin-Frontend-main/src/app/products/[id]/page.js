...
import { PencilIcon, TrashIcon, ArrowLeftIcon, XMarkIcon, EyeIcon, EyeSlashIcon } from '@heroicons/react/24/outline';
...
export default function ProductDetailPage() {
  ...
  const { products, updateProduct, deleteProduct, getStockStatus } = useProducts();
  ...
  // This function toggles the product's published status
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            >
              ...
              {allCategories.find(c => c.name === newCategory.cat)?.subcategories.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}
            </select>
          </div>
        </div>
